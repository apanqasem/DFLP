...
void computeRC(int **permutations, int n, int size, int *uRC, int **pRC) {
  ...
  int *p = new int[n * size];
  int *pr = new int[(n + 2) * (n + 2)];

  for (int m = 0; m < n; m++)
    for (int q = 0; q < size; q++)
      p[m * size + q] = permutations[m][q];

#pragma offload target(mic:0) in(p : length(n * size)) \
  in(uRC: length(size)) out(pr: length((n + 2) * (n + 2)))
  { 
  int i, j, k;
#pragma omp parallel for num_threads(16) private(i,j,k)
    for (i = 1; i < n + 1; i++) {
      ...
      for (j = i + 1; j < n + 1; j++) {
	for (k = 0; k < size; k++) {
	  if (p[(i - 1) * size + k] != p[(j - 1) * size + k]) {
	    cost += uRC[p[(i - 1) * size + k] - 1];
	  }
	}
	pr[(i * (n  + 2)) + j] = pr[(j * (n + 2)) + i] = cost;
	cost = 0;
      }
    }
  }
