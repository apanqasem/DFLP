 ...
 */
void computeRC(int **perms, int n, int size, int *uRC, int **pRC) {
  
  for (int i = 1; i < n + 1; i++) {
    for (int j = i + 1; j < n + 1; j++) {
      int cost = 0;
      for (int k = 0; k < size; k++) 
	cost += (perms[i - 1][k] != perms[j - 1][k]) ? uRC[perms[i - 1][k] - 1] : 0;
...
inline int getRC(int **perms, int *uRC, int n, int size, int i, int j) {
...
void calculateDistance(bool mark[], int distance[], int fact, int previous[],
		       int **perms, int years, int *mhc, int *uRC, int size) {
  ...
  for (i = 0; i < allPerms; i++) {
	...
	if (cunDist >= j * fact + 1 && cunDist <= (j + 1) * fact) {
	  ...
	  for (int i = lb; i < ub; i++) {
	    ...
	    // compute relocation cost between cunRC and iter
	    // this is computed, on-demand, rather than statically to avoid large storage allocation
	    int rCost = 0;
	    // first row and column have nagative RC
	    if (cunRC == 0 || iter == 0) 
	      rCost = -1;
	    // last row and column have negative RC
	    else if (cunRC == fact + 1 || iter == fact + 1) 
	      rCost =  -1;
	    else if (cunRC == iter) 
	      rCost =  0;
	    else {
	      rCost = 0;
	      for (int k = 0; k < size; k++) 
		rCost += (perms[cunRC - 1][k] != perms[iter - 1][k]) ? uRC[perms[cunRC - 1][k] - 1] : 0;
	    }
	    if (rCost >= 0) {
	      if (iter == 0) 
		iter = fact;
	      int dist = distance[cunDist] + rCost + mhc[i];
	      if (distance[i] > dist) {
		distance[i] = dist;
		previous[i] = cunDist;
	      }
...
void optimumPath(int **permutations, int fact, int size, int years,
  ...
  distance[0] = 0;
  calculateDistance(mark, distance, fact, previous, permutations, years, mhc, uRC, size);
...
int main(int argc, char *argv[]) {
  ...
  // derive optimal path 
  optimumPath(permutations, n, size, years, mhc, rcost);
